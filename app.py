...
try:
    ...
    print(json.dumps({{"ok": True, "result": result_str}}))
except Exception as e:
...
'''
    ...
    try:
        ...
        try:
            subprocess.run(['kill', '-9', str(result.pid)], capture_output=True)
        except:
            pass
        return {"ok": False, "error": f"Модуль {module_name} превысил таймаут {timeout}с"}
    except Exception as e:
...
def decide_action():
    ...
    if mood == 'скука' and novelty < 0.5:
        return ('explore', {'type': 'ask_question_to_self'})
    ...
    if mood == 'тревога' and energy > 0.5:
        return ('reflect', {'type': 'self_analysis'})
    ...
    # --- СИСТЕМАТИЧЕСКОЕ ОБУЧЕНИЕ раз в 2 часа ---
    try:
...
@app.route('/')
def home():
    return HTML + '<div style="text-align:center;padding:10px;background:#222;"><a href="/download" style="color:#1a73e8;font-size:14px;text-decoration:none;">Скачать память</a> | <a href="/download-modules" style="color:#1a73e8;font-size:14px;text-decoration:none;">Скачать модули</a> | <a href="/state-view" style="color:#1a73e8;font-size:14px;text-decoration:none;">Состояние</a> | <a href="/status" style="color:#1a73e8;font-size:14px;text-decoration:none;">Статус</a> | <a href="/modules?key=dipkey" style="color:#1a73e8;font-size:14px;text-decoration:none;">Модули</a></div>'
...
@app.route('/status')
def status():
    """Статус бота с информацией о состоянии и потребностях"""
    state = get_state()
