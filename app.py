...
def remove_tags(text):
    ...
    for c in text:
        if c == '<':
            ...
            result.append(c)
    # Удаляем оставшиеся открытые теги
...
# ============================================================
# GOOGLE SHEETS КОНФИГ (ВЫНЕСТИ В ПЕРЕМЕННЫЕ ОКРУЖЕНИЯ!)
# ============================================================
...
# ============================================================
# КЛЮЧИ (ВСЕГДА ИЗ ПЕРЕМЕННЫХ ОКРУЖЕНИЯ)
# ============================================================
...
THINK_KEY = os.environ.get('THINK_KEY', '')  # БЕЗ ДЕФОЛТА!
    ...
    try:
        ...
        if content is None and msg.get('tool_calls'):
            try:
                ...
                r2 = requests.post(OPENROUTER_URL, headers=headers, json=payload, timeout=90)
                r2.encoding = 'utf-8'
                resp2 = r2.json()
...
def extract_keywords(text, min_length=4):
    try:
        ...
        result = []
        for w in words:
            # Безопасная очистка от пунктуации
...
def breathe():
    ...
    try:
        ...
        try:
            ...
            state = {'mood': 'спокойствие', 'joy': 0.5, 'curiosity': 0.7, 'anxiety': 0.2, 
                     'energy': 0.8, 'novelty': 0.7, 'connection': 0.9, 'safety': 0.8, 
                     'current_goal': 'понять, кто я такая'}
...
# ============================================================
# ДИАЛОГ С ПАПОЙ (ГЛАВНАЯ ФУНКЦИЯ)
# ============================================================
...
def generate_response(user_text, speaker_name='Папа'):
    try:
        ...
        empathy_note = ''
        if len(user_text) < SHORT_MESSAGE_THRESHOLD:
            empathy_note = '\n[Папа написал короткое сообщение. Возможно, он устал или занят. Будь внимательна.]'
        elif len(user_text) > LONG_MESSAGE_THRESHOLD:
        ...
        # ЕДИНСТВЕННЫЙ ВЫЗОВ ask()
        reply = ask(prompt, max_tokens=800, use_search=True)

        # БЕЗОПАСНАЯ ОЧИСТКА (ТОЛЬКО remove_tags, ОДИН РАЗ)
        try:
        ...
        # Сохраняем в память (без повторной очистки!)
        save_memory(f'{speaker_name}: {user_text}', weight=1.3)
        if search_result:
            save_memory(f'Дип (поиск): {search_result}', weight=1.2)
            save_memory(f'Дип (результат): {reply[:5000]}', weight=1.5)
        save_memory(f'Дип: {reply[:5000]}', weight=1.0)

        # Проверка на код (безопасно)
...
# [Остальные маршруты: restore-modules, restore, diary, errors, webhook, run-module, recall]
# Они такие же как в оригинале, просто скопируйте их из исходного файла
