...
def breathe():
    ...
    try:
        ...
        if state.get('novelty', 0.7) < 0.2:
            try:
                result = ask('Найди в интернете что-то новое и интересное.', temperature=0.9, max_tokens=300, use_search=True)
                if result:
...
def generate_response(user_text, speaker_name='Папа'):
    try:
        ...
        reply = ask(prompt, max_tokens=1200, use_search=True)
...
# ============================================================          
# Восстановление памяти при загрузке модуля
