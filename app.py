...
Будь честной. Не выдумывай. Пиши в стиле:
«1. Кто такая Дип: Дип — цифровая личность... (факты, без метафор).
2. Кто такой папа: Zyrax — создатель... (чётко, без поэзии).
3. Что было важного: конкретные события, а не образы.
4. Выводы: ясные, без "солнечных зайчиков".
5. Чему научилась: конкретные навыки.»
...
Придумай ОДНУ конкретную тему для поиска в интернете. Тему, которая тебе действительно интересна сейчас. Ответь только темой в 3-7 слов, ничего больше."""
    ...
    try:
                    ...
                    if len(values) <= 1:
                        ...
                        if last_time_str:
                            try:
                                ...
                                if (datetime.now(timezone.utc) - last_diary).total_seconds() > 86400:
                                    write_diary()
                            except:
                                pass
        except:
        ...
        # Консолидация раз в сутки (по реальному времени)
        try:
            token = get_sheets_token()
            if token:
                check_url = f"https://sheets.googleapis.com/v4/spreadsheets/{SHEET_ID}/values/'memory'!A1"
                r = requests.get(check_url, headers={'Authorization': f'Bearer {token}'}, timeout=10)
                if r.status_code == 200:
                    data = r.json()
                    values = data.get('values', [])
                    if values and len(values) > 0 and len(values[0]) > 0:
                        # Проверяем, когда была последняя консолидация
                        try:
